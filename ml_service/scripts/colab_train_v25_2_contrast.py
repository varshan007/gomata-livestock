#!/usr/bin/env python3
"""
colab_train_v25_2_contrast.py
PHASE 25.2: PRODUCTION LEAN CONTRAST-CALIBRATED PEN ENGINE
==============================================================
Provides lean contextual inter-pen structural reasonings for gOmATA,
using contrastive ranking and variance-regularized objectives.
"""
...
logger = logging.getLogger("V25.2.Contrast")
...
class MiniStructuralEncoder(nn.Module):
    ...
    def forward(self, x_node, adj_sub, mask):
        ...
        # Mean pool only, no max
        struct_emb = x2.mean(dim=1)
        return struct_emb
...
class ContextualPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.struct_enc = MiniStructuralEncoder(in_dim=gat_dim, hidden=48, out_dim=48)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                alpha = torch.clamp(self.alpha_param, 0.5, 2.0)
...
def variance_floor(pred, min_std=0.15):
    std = pred.std(unbiased=False)
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 25.2 — Production Lean Contrast-Calibrated Pen Engine")
    logger.info("="*60)
    ...
    logger.info(f"Contextual Structural Fusion Params: {n_trainable:,} (Target < 60k)")
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            loss_rank = pairwise_rank_loss(p_cat, t_cat)
            loss_var = variance_floor(p_cat, min_std=0.15)
            
            loss = (3.0 * loss_mse) + (1.0 * loss_rank) + (0.3 * loss_var)
    ...
    mp = f"{OUT_DIR}/v25_2_contrast_engine.pth"; cp = f"{OUT_DIR}/v25_2_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "25.2", 
        "architecture": "Frozen V21 + Lean Contrast Mesoscopic Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v25_2_contrast_engine.pth")
    logger.info(f"✅ V25.2 Engine saved to {mp} and Drive.")
