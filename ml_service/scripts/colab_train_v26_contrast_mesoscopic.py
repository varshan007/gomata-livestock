#!/usr/bin/env python3
"""
colab_train_v26_contrast_mesoscopic.py
...
logger = logging.getLogger("V26.Contrast")
...
def main():
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            loss_seq = F.mse_loss(p_cat, t_cat)
            loss_rank = pairwise_rank_loss(p_cat, t_cat)
            loss_var = variance_floor(p_cat, min_std=0.05)
            
            loss = (2.0 * loss_seq) + (2.0 * loss_rank) + (0.8 * loss_var)
    ...
    mp = f"{OUT_DIR}/v26_contrast_pen_engine.pth"; cp = f"{OUT_DIR}/v26_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "26.0", 
        "architecture": "Frozen V21 + Contrast Mesoscopic Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v26_contrast_pen_engine.pth")
    logger.info(f"✅ V26 Engine saved to {mp} and Drive.")
