#!/usr/bin/env python3
"""
evaluate_v29_2_interaction_calibrated.py
PHASE 29.2: INTERACTION-CALIBRATED MESOSCOPIC ENGINE
==============================================================
Validates the V29.2 Factorized Structural Epidemic Potential 
calibration against multiplicative regime interactions. Verifies 
metrics across 3 separate random seeds and reports robust correlations.
"""
...
class RegimeInteractionCalibrator(nn.Module):
    def __init__(self):
        super().__init__()
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.outcome_head = RegimeInteractionCalibrator()
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                struct_pred = self.struct_enc(H_feat, A_feat, s_raw) # [1, 1]
                pen_struct_preds[b, p] = struct_pred.squeeze(-1).squeeze(0) # Epidemic Potential mapping directly to mean R0 limits
                
                dr0_mean = bb_out["delta_R0"][b, mask].mean().unsqueeze(0).unsqueeze(0)
                vacc_mean = bb_out["vacc_rank"][b, mask].mean().unsqueeze(0).unsqueeze(0)
                
                s = struct_pred # [1, 1]
                h_int = int_b.unsqueeze(0)
                h_out = outb_b.unsqueeze(0)
                h_bk = bkdn_b.unsqueeze(0)
                
                base = torch.cat([s, h_int, h_out, h_bk, dr0_mean, vacc_mean], dim=-1)
                interactions = torch.cat([s * h_int, s * h_out, s * h_bk, s * dr0_mean], dim=-1)
                calib_input = torch.cat([base, interactions], dim=-1)
                
                outcome = self.outcome_head(calib_input)
...
class PenSimulator:
    ...
    def simulate(self, idx):
        ...
        for p in range(num_pens):
            ...
            # Epidemic Potential proxy — Direct target derived as pure subset limit of `dr0` topology constraints
            if count >= 2:
                pen_struct_tgt[p] = float(dr0[msk].mean())
...
def evaluate_multi_seed(model, device, n_farms=200):
    ...
    global_results = {'dr0': [], 'vacc': [], 'pen': [], 'str': [], 'pen_min': []}
    ...
    for seed in seeds:
        ...
        with torch.no_grad():
            for i in range(n_farms):
                ...
                p_str=o['pen_struct_risk'][0].cpu().numpy(); t_str=L['pen_struct']
                m_s = t_str > 0
                if m_s.sum() >= 2:
                    t_str_val = t_str[m_s]
                    if t_str_val.std(ddof=0) >= 0.05:
                        c_s = safe_pearsonr(t_str_val, p_str[m_s])
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 29.2: INTERACTION-CALIBRATED EPIDEMIC EVALUATION")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v29_2_interaction_calibrated_engine.pth", "models/cattle/v29_2_config.json"),
        ("v29_2_interaction_calibrated_engine.pth", "v29_2_config.json"),
        ("/content/drive/MyDrive/HerdV24/v29_2_interaction_calibrated_engine.pth", "/content/drive/MyDrive/HerdV24/v29_2_config.json"),
    ]
    ...
    print("\n── PEN-LEVEL (Dual Head Prediction) ──")
    ...
    print("\n" + "="*60)
    all_ok = md>=0.98 and mv>=0.92 and res['str_mean']>=0.85 and mp>=0.75 and res['pen_min']>=0.55 and pen_params <= 55000
    if all_ok: print("🏆 PHASE 29.2 COMPLETE — Interaction System Correct")
    else: print("❌ Phase 29.2 FAILED — Constraints broken.")
    print("=" * 60)
