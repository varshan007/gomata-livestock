...
class DualHeadPenEngine(nn.Module):
    ...
    def compute_pen_outcomes(self, pen_struct_basis, herd_ctx, regime_idx):
        ...
        # Herd context (compress 96 -> scalar mean)
        herd_scalar = herd_ctx.mean(dim=-1, keepdim=True)   # [B,1]
...
def main():
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        if not is_stage_b:
            ...
            model.struct_scalar.requires_grad = False
            for p in model.pen_amp_head.parameters(): p.requires_grad = False
        else:
            opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
            model.struct_scalar.requires_grad = True
        ...
        for bi, batch in enumerate(loader):
            ...
            if not is_stage_b:
                ...
                loss = Loss_struct + 2.5 * Loss_outcome + 0.1 * L_var
        ...
        logger.info(f"   ↳ [Telemetry] std_p:{m_out_p.std():.4f} | std_t:{m_out_t.std():.4f}")
    ...
    mp = f"{OUT_DIR}/v30_3_pen_conditional_engine.pth"; cp = f"{OUT_DIR}/v30_3_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "30.3", 
        "architecture": "Pen-Conditional Factorized Epidemic Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v30_3_pen_conditional_engine.pth")
    logger.info(f"✅ V30.3 Engine saved to {mp} and Drive.")
