...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 31 — REGIME-CONDITIONED MONOTONIC SPECTRAL ENGINE")
    logger.info("="*60)
    ...
    opt_outcome = torch.optim.AdamW(model.regime_mlp.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        if not is_stage_b:
            ...
            for p in model.regime_mlp.parameters(): p.requires_grad = False
        else:
            opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
            for p in model.regime_mlp.parameters(): p.requires_grad = True
        ...
        for bi, batch in enumerate(loader):
            ...
            if not is_stage_b:
                ...
                var_floor = F.relu(0.12 - std_pred)
                loss += 0.1 * var_floor
                
                # Phase 31 Global Monotonic Penalty
                if p_mask_global.sum() >= 2:
                    lam1_batch = struct_basis_pred[..., 0] # [B, P]
                    ...
                    monotonic_violation = F.relu(diff_lam * diff_out).mean()
                    
                    mono_loss_batch = monotonic_violation
                    loss += 0.2 * monotonic_violation
