#!/usr/bin/env python3
"""
evaluate_v29_factorized_engine.py
PHASE 29: FACTORIZED EPIDEMIC POTENTIAL + CALIBRATION
==============================================================
Validates the V29 Factorized Structural Epidemic Potential 
...
class DynamicPenCalibrator(nn.Module):
    ...
    def forward(self, struct_pred, h_int, outbrk):
        x = torch.cat([struct_pred, h_int, outbrk], dim=-1)
        return self.net(x)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                # Forward to Dynamic Calibrator — strict 3 parameter intake
                outcome = self.outcome_head(struct_pred, int_b.unsqueeze(0), outb_b.unsqueeze(0))
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 29: FACTORIZED EPIDEMIC POTENTIAL EVALUATION")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v29_factorized_engine.pth", "models/cattle/v29_config.json"),
        ("v29_factorized_engine.pth", "v29_config.json"),
        ("/content/drive/MyDrive/HerdV24/v29_factorized_engine.pth", "/content/drive/MyDrive/HerdV24/v29_config.json"),
    ]
    ...
    print(f"Dual-Head Structural & Baseline Params: {pen_params:,} (Target 35k < X < 55k)")
    ...
    print("\n" + "="*60)
    all_ok = md>=0.98 and mv>=0.92 and res['str_mean']>=0.75 and mp>=0.80 and res['pen_min']>=0.55 and pen_params <= 60000
    if all_ok: print("🏆 PHASE 29 COMPLETE — Factorized Pretraining System Correct")
    else: print("❌ Phase 29 FAILED — Constraints broken.")
    print("=" * 60)
