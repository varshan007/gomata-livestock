...
class PenAmplificationHead(nn.Module):
    ...
    def forward(self, x):
        raw = self.net(x)
        amp = torch.exp(torch.clamp(raw * self.scale, -2.5, 2.5))
        return amp
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.struct_scalar = nn.Sequential(
            nn.Linear(3, 16),
            nn.ReLU(),
            nn.Linear(16, 1),
            nn.Softplus()
    ...
    def compute_pen_outcomes(self, pen_struct_basis, herd_ctx, regime_idx):
        B, P, _ = pen_struct_basis.shape

        # Structural scalar
        struct_scalar = self.struct_scalar(pen_struct_basis).squeeze(-1)  # [B,P]

        # Regime embedding
        regime_emb = self.regime_emb(regime_idx)          # [B,1]
        regime_emb = regime_emb.unsqueeze(1).expand(-1, P, -1)

        # Herd context (compress 96 -> norm magnitude scalar)
        herd_scalar = herd_ctx.norm(dim=-1, keepdim=True)   # [B,1]
        herd_scalar = herd_scalar.unsqueeze(1).expand(-1, P, -1)

        # Amplification input
        amp_input = torch.cat([
            struct_scalar.unsqueeze(-1),
            regime_emb,
            herd_scalar
        ], dim=-1)  # [B,P,3]

        amp_pen = self.pen_amp_head(amp_input).squeeze(-1)  # [B,P]

        # Final outcome
        pen_out = struct_scalar * amp_pen

        return pen_out, amp_pen
        
    def forward(self, ns, adj, pen_map, regime_idx):
        self.backbone.eval()
        with torch.no_grad(): 
        ...
        pen_out, amp_pen = self.compute_pen_outcomes(
            pen_struct_basis,
            bb_out["H_herd"],
            regime_idx
        )
...
def evaluate_model(model, device, n_farms=100, seed=7777):
    ...
    for i in range(n_farms):
        ...
        amp_pred = o['pen_amplification'].squeeze(0).mean()
...
def main():
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        if not is_stage_b:
            ...
            for p in model.struct_scalar.parameters(): p.requires_grad = False
            for p in model.pen_amp_head.parameters(): p.requires_grad = False
        else:
            opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
            for p in model.struct_scalar.parameters(): p.requires_grad = True
        ...
        for bi, batch in enumerate(loader):
            ...
            struct_scalar_val = model.struct_scalar(struct_basis_pred).squeeze(-1) # [B, MAX_PENS]
            ...
            for b_i in range(ns.shape[0]):
                ...
                if p_mask.sum() >= 2:
                    ...
                    amp_pred_c.append(amp_pred_batch[b_i][p_mask].mean().item())
            ...
            if not is_stage_b:
                ...
                loss = Loss_struct + 2.5 * Loss_outcome + 0.5 * L_var
    ...
    mp = f"{OUT_DIR}/v30_3_pen_conditional_fixes_engine.pth"; cp = f"{OUT_DIR}/v30_3_fixes_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "30.3.fixes", 
        "architecture": "Pen-Conditional Factorized Epidemic Engine (Sign Stabilized)",
