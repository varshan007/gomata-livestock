...
class PenAmplificationHead(nn.Module):
    def __init__(self):
        super().__init__()
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.pen_amp_head = PenAmplificationHead()
    ...
    def compute_pen_outcomes(self, pen_struct_basis, herd_ctx, regime_idx, outbreak, vacc_rank, seed_density, temp_factor, vacc_ratio):
        ...
        # Structural embed [B,P,8]
        struct_embed = self.struct_embed(pen_struct_basis)

        # Regime embedding
        regime_emb = self.regime_emb(regime_idx)          # [B,1]
        regime_emb = regime_emb.unsqueeze(1).expand(-1, P, -1)
        
        # Other scalar inputs expanded
        ob_exp = outbreak.view(B, 1, 1).expand(B, P, 1)
        vr_exp = vacc_rank.view(B, 1, 1).expand(B, P, 1)
        sd_exp = seed_density.view(B, 1, 1).expand(B, P, 1)
        tf_exp = temp_factor.view(B, 1, 1).expand(B, P, 1)
        vac_exp = vacc_ratio.view(B, 1, 1).expand(B, P, 1)

        # Amplification input [B,P,6]
        amp_input = torch.cat([
            ob_exp,
            vr_exp,
            regime_emb,
        ...
        dynamics_embed = self.pen_amp_head(amp_input)  # [B,P,8]

        # Final outcome: Bilinear inner product -> Softplus
        raw_out = (struct_embed * dynamics_embed).sum(dim=-1)
        pen_out = F.softplus(raw_out)

        return pen_out, dynamics_embed
...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 30.3 — PEN-CONDITIONAL FACTORIZED EPIDEMIC ENGINE")
    logger.info("="*60)
    ...
    opt_outcome = torch.optim.AdamW(model.pen_amp_head.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        if not is_stage_b:
            ...
            for p in model.struct_embed.parameters(): p.requires_grad = False
            for p in model.pen_amp_head.parameters(): p.requires_grad = False
        else:
            opt = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
            for p in model.struct_embed.parameters(): p.requires_grad = True
            for p in model.pen_amp_head.parameters(): p.requires_grad = True
        ...
        for bi, batch in enumerate(loader):
            ...
            for b_i in range(ns.shape[0]):
                ...
                if p_mask.sum() >= 2:
                    ...
                    amp_pred_c.append(amp_pred_batch[b_i][p_mask].norm(dim=-1).mean().item())
            ...
            if not is_stage_b:
                ...
                Loss_outcome = 1.0 - safe_correlation(outcome_pred[p_mask_global], p_target[p_mask_global])
                # pseudo-supervision: norm(dim=-1) of embedded amp vector compared to amp_tgt_arr
                amp_norm = amp_pred_batch[p_mask_global].norm(dim=-1)
                amp_t_exp = amp_tgt_arr.unsqueeze(1).expand_as(p_target)[p_mask_global]
                Loss_amp = F.mse_loss(amp_norm, amp_t_exp)
                L_var = variance_alignment(outcome_pred[p_mask_global], p_target[p_mask_global])
                loss = Loss_struct + 1.0 * Loss_outcome + 0.5 * Loss_amp + 0.2 * L_var
        ...
        logger.info(f"   ↳ [Telemetry] std_p:{m_out_p.std():.4f} | std_t:{m_out_t.std():.4f} | Amp range: {amp_min:.3f} - {amp_max:.3f}")
    ...
    paths = [
        ("models/cattle/v30_4_physics_corrected_factorized_engine.pth", "models/cattle/v30_4_config.json"),
        ("v30_4_physics_corrected_factorized_engine.pth", "v30_4_config.json"),
        ("/content/drive/MyDrive/HerdV24/v30_4_physics_corrected_factorized_engine.pth", "/content/drive/MyDrive/HerdV24/v30_4_config.json"),
    ]
    torch.save(model.state_dict(), paths[0][0])
    with open(paths[0][1], "w") as f: json.dump({"max_pens":MAX_PENS, "gat_dim":GAT_DIM}, f)
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v30_3_pen_conditional_fixes_engine.pth")
    logger.info(f"✅ V30.3_Fixes Engine saved to {mp} and Drive.")
