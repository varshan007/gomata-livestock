#!/usr/bin/env python3
"""
colab_train_v28_dual_head_mesoscopic.py
PHASE 28: DUAL-HEAD MESOSCOPIC RISK ENGINE
==============================================================
Provides cleanly separated structural topology modeling from epidemiological outcome
learning to avoid supervision mismatch and prevent variance collapse.
"""
...
logger = logging.getLogger("V28.DualHead")
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 28 — Dual-Head Mesoscopic Risk Engine")
    logger.info("="*60)
    ...
    mp = f"{OUT_DIR}/v28_dual_head_engine.pth"; cp = f"{OUT_DIR}/v28_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "28.0", 
        "architecture": "Dual-Head Mesoscopic Risk Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v28_dual_head_engine.pth")
    logger.info(f"✅ V28 Engine saved to {mp} and Drive.")
