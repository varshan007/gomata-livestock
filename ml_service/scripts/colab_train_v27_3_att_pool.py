#!/usr/bin/env python3
"""
colab_train_v27_3_att_pool.py
PHASE 27.3: ATTENTION POOLING ALIGNMENT
==============================================================
Provides lean contextual inter-pen structural reasonings for gOmATA,
using contrastive ranking, variance alignment, and attention pooling.
"""
...
logger = logging.getLogger("V27.3.AttPool")
...
class MiniStructuralEncoder(nn.Module):
    ...
    def forward(self, x_node, adj_sub, mask):
        ...
        struct_emb = torch.cat([att_pool, max_pool], dim=-1)
...
class PenSimulator:
    ...
    def simulate(self, idx):
        ...
        for p in range(num_pens):
            ...
            # Dominance amplification (production safe)
            amplified = np.power(vals, 1.8)

            pen_int[p] = amplified.mean()

        # Global normalization (stable)
        max_val = pen_int.max()
        if max_val > 1e-8:
            pen_int = pen_int / max_val
        else:
            pen_int = np.zeros_like(pen_int)
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 27.3 — Attention Pooling Alignment")
    logger.info("="*60)
    ...
    logger.info(f"Contextual Structural Fusion Params: {n_trainable:,} (Target < 150k)")
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            loss_var = variance_alignment(o['pen_pred'][0], pen_int[0])
            
            loss = (2.0 * loss_mse) + (1.0 * loss_rank) + (0.5 * loss_var)
    ...
    mp = f"{OUT_DIR}/v27_3_att_pool_engine.pth"; cp = f"{OUT_DIR}/v27_3_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "27.3", 
        "architecture": "Frozen V21 + Attention-Pooled Structural Context",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v27_3_att_pool_engine.pth")
    logger.info(f"✅ V27.3 Engine saved to {mp} and Drive.")
