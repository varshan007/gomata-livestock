#!/usr/bin/env python3
"""
evaluate_v29_1_regime_calibrated.py
PHASE 29.1: REGIME-AWARE CALIBRATED MESOSCOPIC ENGINE
==============================================================
Validates the V29.1 Regime-Aware Structural Epidemic Potential 
calibration against dynamic regimes. Verifies metrics across 
3 separate random seeds and reports robust correlations.
"""
...
class DynamicPenCalibrator(nn.Module):
    ...
    def forward(self, struct_pred, h_int, outbrk, bkdn, dr0_m, vacc_m):
        x = torch.cat([struct_pred, h_int, outbrk, bkdn, dr0_m, vacc_m], dim=-1)
        return self.net(x)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                # Forward to Dynamic Calibrator — strict 6 parameter regime context
                outcome = self.outcome_head(struct_pred, int_b.unsqueeze(0), outb_b.unsqueeze(0), bkdn_b.unsqueeze(0), dr0_mean, vacc_mean)
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 29.1: REGIME-AWARE FACTORIZED EPIDEMIC EVALUATION")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v29_1_regime_calibrated_engine.pth", "models/cattle/v29_1_config.json"),
        ("v29_1_regime_calibrated_engine.pth", "v29_1_config.json"),
        ("/content/drive/MyDrive/HerdV24/v29_1_regime_calibrated_engine.pth", "/content/drive/MyDrive/HerdV24/v29_1_config.json"),
    ]
    ...
    print(f"Dual-Head Structural & Baseline Params: {pen_params:,} (Target < 35k)")
    ...
    print("\n" + "="*60)
    all_ok = md>=0.98 and mv>=0.92 and res['str_mean']>=0.82 and mp>=0.75 and res['pen_min']>=0.55 and pen_params <= 35000
    if all_ok: print("🏆 PHASE 29.1 COMPLETE — Regime-Aware Contrast System Correct")
    else: print("❌ Phase 29.1 FAILED — Constraints broken.")
    print("=" * 60)
