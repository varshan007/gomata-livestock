#!/usr/bin/env python3
"""
evaluate_v30_factorized_dynamics.py
PHASE 30: FACTORIZED EPIDEMIC DYNAMICS ENGINE
==============================================================
Validates the V30 Factorized Structural Epidemic Physics
...
class DualHeadPenEngine(nn.Module):
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            # Using simple global context mappings for evaluate passing down manually later
            pass
    ...
    for seed in seeds:
        ...
        with torch.no_grad():
            for i in range(n_farms):
                ...
                p_str=o['pen_struct_risk'].squeeze(0); t_str=L['pen_struct']
                ...
                p_out = torch.zeros_like(p_str)
                p_out = model.scale_param * (p_str * amp_pred[0])
                
                # Arrays back to numpy securely for strict tests
                p_str = p_str.cpu().numpy()
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 30: FACTORIZED EPIDEMIC EVALUATION")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v30_factorized_dynamics_engine.pth", "models/cattle/v30_config.json"),
        ("v30_factorized_dynamics_engine.pth", "v30_config.json"),
        ("/content/drive/MyDrive/HerdV24/v30_factorized_dynamics_engine.pth", "/content/drive/MyDrive/HerdV24/v30_config.json"),
    ]
    ...
    if all_ok: print("🏆 PHASE 30 COMPLETE — Valid Factorization Model Generated")
    else: print("❌ Phase 30 FAILED — Constraints broken.")
    print("=" * 60)
