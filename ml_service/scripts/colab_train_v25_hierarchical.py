#!/usr/bin/env python3
"""
colab_train_v25_hierarchical.py
PHASE 25: HIERARCHICAL INTER-PEN TRANSFORMER
==============================================================
Provides contextual inter-pen structural reasonings for gOmATA.
Fuses isolated subgraph embeddings across farm contexts
using a Multi-Head scaled self-attention mechanism.
"""
...
logger = logging.getLogger("V25.Hierarchical")
...
class MiniStructuralEncoder(nn.Module):
    def __init__(self, in_dim=96, hidden=12, out_dim=48):
        super().__init__()
        self.gat1 = PenGATLayer(in_dim, hidden, heads=4, concat=True)
        self.norm1 = nn.LayerNorm(hidden * 4)
        
        self.gat2 = PenGATLayer(hidden * 4, hidden * 2, heads=2, concat=True)
        self.norm2 = nn.LayerNorm(hidden * 4)
        
        self.proj_out = nn.Linear(hidden * 4, out_dim)
    ...
    def forward(self, x_node, adj_sub, mask):
        ...
        x2 = self.norm2(x2)
        x_out = self.proj_out(x2)
        
        x_out = torch.nan_to_num(x_out, 0.0) 
        
        # Concat Pool [Mean, Max]
        mean_pool = x_out.mean(dim=1)
        max_pool = x_out.max(dim=1).values
        struct_emb = torch.cat([mean_pool, max_pool], dim=-1)
        return struct_emb

def expand_struct_features(S_vec):
    quad_terms = []
    for i in range(6):
        for j in range(i+1, 6):
            quad_terms.append(S_vec[i] * S_vec[j])
    quad = np.stack(quad_terms, axis=-1)
    return np.concatenate([S_vec, quad], axis=-1)

def extract_structural_features(adj_sub, max_cows):
    """
    Computes S_pen token [21] (expanded) and Top-3 Laplacian Eigenvectors
    """
    N = adj_sub.shape[0]
    if N < 2: return torch.zeros(21, dtype=torch.float32), torch.zeros(1, 3, dtype=torch.float32)
    ...
    vec_expanded = expand_struct_features(vec)
    
    return torch.tensor(vec_expanded, dtype=torch.float32), torch.tensor(top3_vecs, dtype=torch.float32)

class PenTransformer(nn.Module):
    def __init__(self, in_dim=96, dim=32, heads=4, layers=2):
        super().__init__()
...
class ContextualPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.struct_enc = MiniStructuralEncoder(in_dim=gat_dim, hidden=12, out_dim=48)
        
        self.pen_transformer = PenTransformer(in_dim=96, dim=32, heads=4, layers=2)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        E_stack = torch.zeros(B, self.max_pens, self.gat_dim, device=ns.device, dtype=ns.dtype)
        valid_mask = torch.ones(B, self.max_pens, dtype=torch.bool, device=ns.device) # True = ignore
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                valid_mask[b, p] = False # Mark as Valid (False to ignore)
                ...
                alpha = torch.clamp(self.alpha_param, 0.5, 3.0)
                ...
                struct_emb = struct_emb.squeeze(0) # [96]
                
                E_stack[b, p] = struct_emb
                
        # Inter-Pen Reasoning
        H_farm = self.pen_transformer(E_stack, valid_mask) # [B, P, 96]
        
        # Scoring
        for b in range(B):
            for p in range(self.max_pens):
                if not valid_mask[b, p]:
                    pen_preds[b, p] = self.pen_head(H_farm[b, p]).squeeze(-1)
                
        mean_att = 0.0; max_att = 0.0 # Tracking legacy metrics for struct simplicity
...
def compute_pen_mse(pred_pen, true_pen):
    B = pred_pen.shape[0]
    preds, targets = [], []
    ...
    if not preds: return torch.tensor(0.0, device=pred_pen.device, requires_grad=True)
    
    preds_cat = torch.cat(preds)
    targets_cat = torch.cat(targets)
    return F.mse_loss(preds_cat, targets_cat)
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 24.0 — Pure Mesoscopic Pen Head")
    logger.info("="*60)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            p_cat, t_cat = get_valid_pens(o['pen_pred'], pen_int)
            
            if p_cat.numel() < 2:
                opt.zero_grad(set_to_none=True)
                continue
                
            loss_mse = F.mse_loss(p_cat, t_cat)
            loss_corr = 1.0 - safe_correlation(p_cat, t_cat)
            
            std_p = p_cat.std(unbiased=False)
            loss_var = F.relu(0.12 - std_p)
            
            loss = (3.0 * loss_mse) + (0.8 * loss_corr) + (0.2 * loss_var)
        ...
        with torch.no_grad():
            ...
            alpha_val = out_s.get("alpha", 1.0)
            mean_att = out_s.get("mean_att", 0.0)
            max_att = out_s.get("max_att", 0.0)
        ...
        logger.info(f"EP{ep+1} | L:{tr_loss/len(loader):.3f} | ΔR₀:{m['dr0']:.4f} Vacc:{m['vacc']:.4f} Pen:{m['pen_mean']:.3f} α:{alpha_val:.3f}")
    ...
    mp = f"{OUT_DIR}/v25_hierarchical_engine.pth"; cp = f"{OUT_DIR}/v25_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "25.0", 
        "architecture": "Frozen V21 Backbone + Hierarchical Pen Transformer Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v25_hierarchical_engine.pth")
    logger.info(f"✅ V25.0 Engine saved to {mp} and Drive.")
