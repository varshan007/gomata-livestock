#!/usr/bin/env python3
"""
colab_train_v30_factorized_dynamics.py
PHASE 30: FACTORIZED EPIDEMIC DYNAMICS ENGINE
==============================================================
Production Grade Physics-Aligned Factorization.
Outcome ≈ Structural Potential × Amplification Factor.
"""
...
class StructuralPenEncoder(nn.Module):
    def __init__(self, in_dim=96, hidden=48, out_dim=48):
        ...
        self.proj = nn.Sequential(nn.Linear(out_dim, 32), nn.GELU(), nn.Linear(32, 1))
...
class AmplificationHead(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(103, 32), # 96 (H_herd) + 4 (Regime) + 3 (Scalars: Seed, Temp, Vacc)
            nn.GELU(),
            nn.Linear(32, 1),
            nn.Softplus()
        )
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.struct_enc = StructuralPenEncoder(in_dim=gat_dim, hidden=48, out_dim=48)
        self.regime_emb = nn.Embedding(4, 4)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        pen_amp_preds = torch.zeros(B, self.max_pens, device=ns.device, dtype=ns.dtype)
        ...
        for b in range(B):
            ...
            # Extract basic simulator targets out of state variables if explicitly bound
            # In training, these are fed in parallel with outcome arrays per-farm
            pass # See iteration down specific graph
            ...
            for p in range(self.max_pens):
                ...
                struct_pred = torch.sigmoid(self.struct_enc(H_feat, A_feat, s_raw)) # [1, 1] - exactly bounded
                pen_struct_preds[b, p] = struct_pred.squeeze(-1).squeeze(0)
            
            # Outcome mapping for purely evaluative states (training does this differently block by block)
            if not self.training:
                # Default baseline evaluation context bindings (no specific farm scalars since evaluate passes just structures without explicit data points)
                r_emb = self.regime_emb(torch.tensor([0], device=ns.device))
                amp_input = torch.cat([H_herd, r_emb, torch.tensor([[0.05]], device=ns.device), torch.tensor([[0.0]], device=ns.device), torch.tensor([[0.0]], device=ns.device)], dim=-1)
                amp_pred = self.outcome_head(amp_input).squeeze(-1)
                pen_outcome_preds[b] = self.scale_param * (pen_struct_preds[b] * amp_pred[0])
    ...
    def simulate(self, idx):
        ...
        for p in range(num_pens):
            ...
            if count >= 2:
                ...
                try: 
                    evals = np.linalg.eigvals(lap)
                    sr = float(np.max(np.abs(evals)))
                    pen_struct_tgt[p] = sr / count
                except: 
                    pen_struct_tgt[p] = 0.0
...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 30 — FACTORIZED EPIDEMIC DYNAMICS ENGINE")
    logger.info("="*60)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        # Scale_param frozen in stage A
        opt_a = torch.optim.AdamW([
            {'params': model.struct_enc.parameters()},
            {'params': model.regime_emb.parameters()},
            {'params': model.outcome_head.parameters()}
        ], lr=3e-4, weight_decay=1e-4)
        
        # Full unfreeze in Stage B
        opt_b = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4)
        
        if is_stage_b:
            model.scale_param.requires_grad = True

        opt = opt_b if is_stage_b else opt_a
        ...
        for bi, batch in enumerate(loader):
            ...
            struct_pred = o['pen_struct_risk']
            H_herd = o['H_herd'] # [B, 96]
            
            r_emb = model.regime_emb(rgm_i_arr) # [B, 4]
            amp_input = torch.cat([H_herd, r_emb, rsd_arr, rtf_arr, rvr_arr], dim=-1) # [B, 103]
            amp_pred = model.outcome_head(amp_input).squeeze(-1) # [B]
            
            outcome_pred = torch.zeros_like(struct_pred)
            for b_i in range(ns.shape[0]):
                outcome_pred[b_i] = model.scale_param * (struct_pred[b_i] * amp_pred[b_i])
                
            Loss_struct = 0.0; Loss_outcome = 0.0
            p_mask_all = []
            ...
            for b_i in range(ns.shape[0]):
                ...
                if p_mask.sum() >= 2:
                    ...
                    s_cat.extend(struct_pred[b_i][p_mask].cpu().detach().numpy())
                    s_t_cat.extend(s_target_arr[b_i][p_mask].cpu().numpy())
                    out_p.extend(outcome_pred[b_i][p_mask].cpu().detach().numpy())
                    out_t.extend(p_target[b_i][p_mask].cpu().numpy())
                    amp_pred_c.append(amp_pred[b_i].item())
                    amp_tgt_c.append(amp_tgt_arr[b_i].item())
                else:
                    p_mask_all.append(False)
                    
            p_mask_all = torch.tensor(p_mask_all, device=device)
            p_mask_global = p_target > 0
            
            if p_mask_global.sum() < 2: continue
            
            Loss_struct = F.mse_loss(struct_pred[p_mask_global], s_target_arr[p_mask_global])
            Loss_amp = F.mse_loss(amp_pred[p_mask_all], amp_tgt_arr[p_mask_all]) if p_mask_all.sum() > 0 else 0.0
            Loss_outcome = F.mse_loss(outcome_pred[p_mask_global], p_target[p_mask_global])
            
            loss = 2.0 * Loss_struct + 1.5 * Loss_amp + 3.0 * Loss_outcome
        ...
        logger.info(f"   ↳ [Telemetry] std_p:{m_out_p.std():.4f} | std_t:{m_out_t.std():.4f} | Sc:{model.scale_param.item():.3f}")
    ...
    mp = f"{OUT_DIR}/v30_factorized_dynamics_engine.pth"; cp = f"{OUT_DIR}/v30_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "30.0", 
        "architecture": "Factorized Epidemic Dynamics Engine",
        "struct_corr": float(m['str_mean']), "amp_corr": float(m['amp_mean']),
        "outcome_corr": float(m['pen_mean']),
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v30_factorized_dynamics_engine.pth")
    logger.info(f"✅ V30 Engine saved to {mp} and Drive.")
