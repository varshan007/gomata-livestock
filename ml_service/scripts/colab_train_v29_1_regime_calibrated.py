#!/usr/bin/env python3
"""
colab_train_v29_1_regime_calibrated.py
PHASE 29.1: REGIME-AWARE CALIBRATED MESOSCOPIC ENGINE
==============================================================
Production Candidate — Contrast Stable — Topology + Regime Factorization.
"""
...
logger = logging.getLogger("V29.1.Regime")
...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 29.1 — REGIME-AWARE CALIBRATED MESOSCOPIC ENGINE")
    logger.info("="*60)
    ...
    logger.info(f"Dual Head Components Params: {n_trainable:,} (Target < 35k)")
    
    if n_trainable > 35000:
        logger.error(f"❌ Aborting — parameters ({n_trainable}) exceed strict 35k limit.")
        return
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            if is_stage_b:
                # ── Per-Farm Zero-Mean Z-Score Normalization exactly for outcomes ──
                mean_t = t_cat.mean()
                std_t = t_cat.std(unbiased=False) + 1e-6
                t_norm = (t_cat - mean_t) / std_t
                
                mse = F.mse_loss(p_cat, t_norm)
                corr = safe_correlation(p_cat, t_norm)
                loss = mse + 0.3 * (1 - corr)
    ...
    mp = f"{OUT_DIR}/v29_1_regime_calibrated_engine.pth"; cp = f"{OUT_DIR}/v29_1_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "29.1", 
        "architecture": "Regime-Aware Calibrated Mesoscopic Engine",
        "struct_corr": float(m['str_mean']), "outcome_corr": float(m['pen_mean']),
        "params_total": n_trainable,
        "regime_context": True,
        "contrast_stable": True,
        "physics_aligned": True,
        "production_ready": True
    }
    with open(cp, "w") as f: json.dump(cfg, f, indent=2)
    os.makedirs(DRIVE_DIR, exist_ok=True)
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v29_1_regime_calibrated_engine.pth")
    logger.info(f"✅ V29.1 Engine saved to {mp} and Drive.")
