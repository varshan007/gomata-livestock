#!/usr/bin/env python3
"""
colab_train_v27_contrast_supervision.py
PHASE 27: CONTRAST-ALIGNED SIMULATOR SUPERVISION
==============================================================
...
logger = logging.getLogger("V27.Contrast")
...
def variance_floor(pred, min_std=0.05):
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 27 — Contrast-Aligned Simulator Supervision")
    logger.info("="*60)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            loss = (2.0 * loss_mse) + (1.0 * loss_rank)
    ...
    mp = f"{OUT_DIR}/v27_supervised_engine.pth"; cp = f"{OUT_DIR}/v27_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "27.0", 
        "architecture": "Frozen V21 + Contrast-Aligned Simulator Supervision",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v27_supervised_engine.pth")
    logger.info(f"✅ V27 Engine saved to {mp} and Drive.")
