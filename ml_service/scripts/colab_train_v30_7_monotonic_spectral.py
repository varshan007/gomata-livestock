...
class SpectralScaler(nn.Module):
    def __init__(self, scale=10.0):
        super().__init__()
        self.scale = scale

    def forward(self, struct_basis):
        # struct_basis: [B, P, 3]
        lam1 = struct_basis[..., 0]
        lam2 = struct_basis[..., 1]
        deg_density = struct_basis[..., 2]

        lam1_scaled = torch.tanh(lam1 / self.scale)
        lam2_scaled = torch.tanh(lam2 / (self.scale * 0.5))
        deg_scaled = torch.tanh(deg_density)

        return torch.stack([lam1_scaled, lam2_scaled, deg_scaled], dim=-1)

class StableInteractionHead(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, struct_feat, dyn_feat):
        # struct_feat: [B, P, 3]
        # dyn_feat:    [B, 4]

        B, P, _ = struct_feat.shape
        dyn_exp = dyn_feat.unsqueeze(1).expand(B, P, -1)

        x = torch.cat([struct_feat, dyn_exp], dim=-1)

        raw = self.net(x).squeeze(-1)

        raw = torch.clamp(raw, -8.0, 8.0)
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        # Phase 30.7 Monotonic Architecture Constraints
        self.struct_weight = nn.Parameter(torch.tensor([1.0]))
        self.struct_other = nn.Linear(2, 1, bias=False)
        
        self.regime_emb = nn.Embedding(4, 1)
        self.spectral_scaler = SpectralScaler(scale=10.0)
        self.interaction_head = StableInteractionHead()
        
    def compute_pen_outcomes(self, pen_struct_basis, herd_ctx, regime_idx, outbreak, vacc_rank, seed_density, temp_factor, vacc_ratio):
        B, P, _ = pen_struct_basis.shape

        # Enforce Monotonic Physical Scaling First
        lam1 = pen_struct_basis[..., 0:1]
        others = pen_struct_basis[..., 1:3]
        positive_weight = torch.abs(self.struct_weight)
        
        s_scalar = positive_weight * lam1 + self.struct_other(others)
        
        # Override the scale input to represent the structurally clamped output with monotonic integrity
        # Note: In phase 30.7, we embed the s_scalar inside the spectral scaler logic 
        # But wait, SpectralScaler expects 3 variables. The user explicitly requested:
        # "Instead of s_scalar = self.struct_scalar(pb) Use lam1 = pb... s_scalar = positive... 
        # But Phase 30.6 Spectral Scaler uses [lam1, lam2, deg].
        # Since struct_scalar produces a 1D, we override the spectral scaler to process the combined basis.
        
        # Let's map it safely so we don't break the stable interaction head (which expects 3 features).
        # We will apply the Monotonic Constraint to lam1 specifically before scaling:
        lam1_mono = positive_weight * lam1 + self.struct_other(others)

        # Spectral scaling with monotonic lam1
        lam1_scaled = torch.tanh(lam1_mono / self.spectral_scaler.scale).squeeze(-1)
        lam2_scaled = torch.tanh(others[..., 0] / (self.spectral_scaler.scale * 0.5))
        deg_scaled = torch.tanh(others[..., 1])
        struct_scaled = torch.stack([lam1_scaled, lam2_scaled, deg_scaled], dim=-1)

        # Dynamic features
        dyn_input = torch.cat([
            outbreak.view(B, 1),
            vacc_rank.view(B, 1),
            self.regime_emb(regime_idx).view(B, 1),
            temp_factor.view(B, 1)
        ], dim=-1)
        
        pen_out = self.interaction_head(struct_scaled, dyn_input)

        return pen_out, dyn_input
    ...
    def forward(self, ns, adj, pen_map, regime_idx, **kwargs):
        ...
        pen_out, dynamics_embed = self.compute_pen_outcomes(
            pen_struct_basis,
            bb_out["H_herd"],
            regime_idx,
            bb_out["outbreak"],
            bb_out["vacc_rank"].mean(dim=1),
            kwargs.get("seed_density", torch.zeros_like(regime_idx).float()),
            kwargs.get("temp_factor", torch.zeros_like(regime_idx).float()),
            kwargs.get("vacc_ratio", torch.zeros_like(regime_idx).float())
        )
...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 30.7 — MONOTONIC SPECTRAL ENGINE")
    logger.info("="*60)
    ...
    opt_outcome = torch.optim.AdamW(model.interaction_head.parameters(), lr=1e-4, weight_decay=1e-4)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        if not is_stage_b:
            ...
            for p in model.interaction_head.parameters(): p.requires_grad = True
        ...
        for bi, batch in enumerate(loader):
            ...
            if not is_stage_b:
                ...
                loss = 1.0 * Loss_struct + 1.5 * Loss_outcome
                
                std_pred = outcome_pred[p_mask_global].std(unbiased=False)
                var_floor = F.relu(0.15 - std_pred)
                loss += 0.3 * var_floor
                
                # Phase 30.7 Monotonic Constraint Batch Penalty
                for b_i in range(ns.shape[0]):
                    pmb = p_target[b_i] > 0
                    if pmb.sum() > 2:
                        lam1_vals = struct_basis_pred[b_i][pmb][:, 0]
                        lam1_sorted, idx = torch.sort(lam1_vals)
                        pen_sorted = outcome_pred[b_i][pmb][idx]
                        
                        diff_pen = pen_sorted[1:] - pen_sorted[:-1]
                        diff_lam = lam1_sorted[1:] - lam1_sorted[:-1]
                        mask = diff_lam > 1e-4
                        
                        if mask.sum() > 0:
                            monotonic_violation = torch.clamp(-diff_pen[mask], min=0.0)
                            mono_loss_batch += monotonic_violation.mean()
                            
                loss += 0.5 * mono_loss_batch
        ...
        mono_metric = mono_loss_batch.item() if isinstance(mono_loss_batch, torch.Tensor) else mono_loss_batch
        ...
        logger.info(f"   ↳ [Telemetry] std_p:{m_out_p.std():.4f} | std_t:{m_out_t.std():.4f} | Monotonic Violations: {mono_metric:.4f}")
    ...
    paths = [
        ("models/cattle/v30_7_monotonic_spectral_engine.pth", "models/cattle/v30_7_config.json"),
        ("v30_7_monotonic_spectral_engine.pth", "v30_7_config.json"),
        ("/content/drive/MyDrive/HerdV24/v30_7_monotonic_spectral_engine.pth", "/content/drive/MyDrive/HerdV24/v30_7_config.json"),
    ]
    ...
    logger.info(f"✅ V30.7 Engine saved to {paths[0][0]}")
