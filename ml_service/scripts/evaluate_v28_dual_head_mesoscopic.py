#!/usr/bin/env python3
"""
evaluate_v28_dual_head_mesoscopic.py
PHASE 28: DUAL-HEAD MESOSCOPIC RISK ENGINE
==============================================================
Validates the V28 Dual-Head Structural Pretraining and Dynamic Calibration.
...
class StructuralPenEncoder(nn.Module):
    def __init__(self, in_dim=96, hidden=24, out_dim=48):
        super().__init__()
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.struct_enc = StructuralPenEncoder(in_dim=gat_dim, hidden=24, out_dim=48)
    ...
    def forward(self, ns, adj, pen_map):
        ...
        for b in range(B):
            ...
            for p in range(self.max_pens):
                ...
                pen_struct_preds[b, p] = struct_emb.mean()
    ...
    def simulate(self, idx):
        ...
        for p in range(num_pens):
            ...
            if count >= 2:
                ...
                try: 
                    L = np.diag(degs) - A_p
                    _, evs = np.linalg.eigh(L)
                    eig_r = float(np.real(evs[-1])) if len(evs) > 0 else 0.0
                except: eig_r = 0.0
                pen_struct_tgt[p] = m_deg + m_clust + eig_r

        # Normalizations
        max_int = pen_int.max()
        pen_int = pen_int / max_int if max_int > 1e-8 else np.zeros_like(pen_int)
        
        max_str = pen_struct_tgt.max()
        pen_struct_tgt = pen_struct_tgt / max_str if max_str > 1e-8 else np.zeros_like(pen_struct_tgt)
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 28: DUAL-HEAD MESOSCOPIC RISK ENGINE EVALUATION")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v28_dual_head_engine.pth", "models/cattle/v28_config.json"),
        ("v28_dual_head_engine.pth", "v28_config.json"),
        ("/content/drive/MyDrive/HerdV24/v28_dual_head_engine.pth", "/content/drive/MyDrive/HerdV24/v28_config.json"),
    ]
    ...
    print(f"Dual-Head Structural & Baseline Params: {pen_params:,} (Target 60k < X < 90k)")
    ...
    print("\n" + "="*60)
    all_ok = md>=0.98 and mv>=0.92 and res['str_mean']>=0.85 and mp>=0.80 and res['pen_min']>=0.55 and pen_params <= 90000
    if all_ok: print("🏆 PHASE 28 COMPLETE — Dual-Head Pretraining System Correct")
    else: print("❌ Phase 28 FAILED — Constraints broken.")
    print("=" * 60)
