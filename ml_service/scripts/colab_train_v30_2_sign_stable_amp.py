...
def evaluate_model(model, device, n_farms=100, seed=7777):
    ...
    for i in range(n_farms):
        ...
        s_scalar_val = model.struct_scalar(o['pen_struct_basis']).squeeze(-1).squeeze(0) # [MAX_PENS]
...
def main():
    logger.info("="*60)
    logger.info("🧠 PHASE 30.2 — SIGN-STABLE AMPLIFICATION FACTORIZATION")
    logger.info("="*60)
    ...
    for ep in range(STAGE_A_EP + STAGE_B_EP):
        ...
        for bi, batch in enumerate(loader):
            ...
            # Use 1D inputs directly mapped via 4 feature dimensions for lightweight amplification
            outb_arr = o["outbreak"]
            vacc_arr = o["vacc_rank"].mean(dim=1)
            r_emb = model.regime_emb(rgm_i_arr).squeeze(-1)
            opt_s = model.opt_scalar.expand(ns.shape[0])
            amp_input = torch.stack([outb_arr, vacc_arr, r_emb, opt_s], dim=-1) # [B, 4]
            amp_pred = model.outcome_head(amp_input).squeeze(-1) # [B]
            
            outcome_pred = torch.zeros_like(struct_scalar_val)
            for b_i in range(ns.shape[0]):
                outcome_pred[b_i] = struct_scalar_val[b_i] * amp_pred[b_i]
    ...
    mp = f"{OUT_DIR}/v30_2_sign_stable_amp_engine.pth"; cp = f"{OUT_DIR}/v30_2_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "30.2", 
        "architecture": "Sign-Stable Amplification Factorization Engine",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v30_2_sign_stable_amp_engine.pth")
    logger.info(f"✅ V30.2 Engine saved to {mp} and Drive.")
