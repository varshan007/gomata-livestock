#!/usr/bin/env python3
"""
evaluate_v22_residual_safe.py (STANDALONE)
Phase 22.1 Spectrally-Safe Residual Evaluation
======================================
Self-contained. Only needs: v22_residual_safe.pth + v22_config.json
...
class HerdEngineV21_Backbone(nn.Module):
    ...
    def forward(self, ns, adj, pen_map):
        ...
        return {
            "H_node": H_node, "H_herd": H_herd, "delta_R0": dr0,
...
# ═══════════════════════════════════════════════════════════════
# PHASE 22.1 RESIDUAL PEN ENGINE
# ═══════════════════════════════════════════════════════════════

class ResidualPenHead(nn.Module):
    def __init__(self, pen_feat_dim=102):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(pen_feat_dim, 64), nn.LayerNorm(64), nn.GELU(), nn.Linear(64, 1))
        self.gate_param = nn.Parameter(torch.tensor(-3.0))

    def forward(self, pen_features):
        residual = self.net(pen_features)
...
class SafeResidualEngine(nn.Module):
    ...
    def forward(self, ns, adj, pen_map):
        ...
        pen_feats, _ = self._compute_pen_features(bb_out["H_node"], adj, pen_map)
        residual_pen, gate = self.residual_head(pen_feats)
...
def evaluate_v22_safe(model_path, config_path, n_farms=200, seed=9999):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("=" * 60)
    print("🧠 PHASE 22.1: SPECTRALLY-SAFE RESIDUAL EVALUATION")
    print("=" * 60)
    ...
    print("\n── PEN-LEVEL (Safe Residual Correction) ──")
    print(f"  {tag(mp,0.80)} Pen Mean Corr:     {mp:.4f} ± {np.std(pen_c):.4f}  (≥ 0.80)")
    print(f"  {'✅' if min_p >= 0.50 else '⚠️'} Pen Min Corr:      {min_p:.4f}   (≥ 0.50)")
    print(f"  {'✅' if gate_val < 0.25 else '⚠️'} Initial Gate Val:  {gate_val:.3f} (Expected << 0.25)")

    print("\n" + "="*60)
    all_ok = oa>=0.98 and ba>=0.95 and ic>=0.90 and md>=0.97 and mv>=0.92 and mp>=0.80
    if all_ok: print("🏆 PHASE 22.1 COMPLETE — Spectrally-Safe Residual Correction Active")
    elif md >= 0.97 and mp >= 0.70: print("✅ Phase 22.1 acceptable — Target pen correlation ~80% tracking")
    else: print("❌ Phase 22.1 REJECTED — Targets not met")
    print("=" * 60)

if __name__ == "__main__":
    paths = [
        ("models/cattle/v22_residual_safe.pth", "models/cattle/v22_config.json"),
        ("v22_residual_safe.pth", "v22_config.json"),
        ("/content/models/cattle/v22_residual_safe.pth", "/content/models/cattle/v22_config.json"),
        ("/content/drive/MyDrive/HerdV22/v22_residual_safe.pth", "/content/drive/MyDrive/HerdV22/v22_config.json"),
    ]
    for mp, cp in paths:
        if os.path.exists(mp) and os.path.exists(cp):
            print(f"Found: {mp}"); evaluate_v22_safe(mp, cp, n_farms=200); break
    else:
