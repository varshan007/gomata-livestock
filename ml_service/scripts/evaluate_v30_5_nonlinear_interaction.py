...
class NonlinearInteractionHead(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        # NEW: nonlinear interaction capacity
    ...
    def forward(self, struct_basis, dyn_inputs):
        """
        struct_basis: [B, P, 3]
        dyn_inputs:   [B, 4]
        """
        B, P, _ = struct_basis.shape
        
        s = self.struct_proj(struct_basis)              # [B,P,8]
        d = self.dyn_proj(dyn_inputs).unsqueeze(1)      # [B,1,8]
        
        interaction = s * d                             # [B,P,8]
        
        raw = self.interaction_mlp(interaction).squeeze(-1)  # [B,P]
        
        # Physics: amplification must be ≥ 0
...
class DualHeadPenEngine(nn.Module):
    def __init__(self, backbone, max_pens=MAX_PENS, gat_dim=GAT_DIM):
        ...
        self.interaction_head = NonlinearInteractionHead()
    ...
    def compute_pen_outcomes(self, pen_struct_basis, herd_ctx, regime_idx, outbreak, vacc_rank, seed_density, temp_factor, vacc_ratio):
        ...
        # Dynamic scalars tracking logic to form 4D dyn inputs
        dyn_input = torch.cat([
            outbreak.view(B, 1),
            vacc_rank.view(B, 1),
...
def evaluate_multi_seed(model, device, n_farms=200):
    ...
    print(f"\n✅ All Phase 30.5 Evaluation Target Outcomes Verified on 10,000 Farms.")
    model.eval()
...
def main():
    ...
    print("=" * 60)
    print("🧠 PHASE 30.5: NONLINEAR INTERACTION FACTORIZED EPIDEMIC ENGINE")
    print("=" * 60)
    print(f"Device: {device}")

    paths = [
        ("models/cattle/v30_5_nonlinear_interaction_factorized_engine.pth", "models/cattle/v30_5_config.json"),
        ("v30_5_nonlinear_interaction_factorized_engine.pth", "v30_5_config.json"),
        ("/content/drive/MyDrive/HerdV24/v30_5_nonlinear_interaction_factorized_engine.pth", "/content/drive/MyDrive/HerdV24/v30_5_config.json"),
    ]
    ...
    if all_ok: print("🏆 PHASE 30.5 COMPLETE — Valid Physics Corrected Engine Generated")
    else: print("❌ Phase 30.5 FAILED — Constraints broken.")
    print("=" * 60)
