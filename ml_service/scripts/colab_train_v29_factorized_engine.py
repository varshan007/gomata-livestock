#!/usr/bin/env python3
"""
colab_train_v29_factorized_engine.py
PHASE 29: FACTORIZED EPIDEMIC POTENTIAL + CALIBRATION
==============================================================
Forces strict physics alignment: Epidemic Potential (Topology)
factors explicitly against Dynamic Regime (Scale and Outcomes).
"""
...
logger = logging.getLogger("V29.Factorized")
...
def main():
    logger.info("="*60)
    logger.info("🧠 Phase 29 — Factorized Epidemic Potential + Outcome Engine")
    logger.info("="*60)
    ...
    logger.info(f"Dual Head Components Params: {n_trainable:,} (Target 35k < X < 55k)")
    
    if n_trainable > 60000:
        logger.error(f"❌ Aborting — parameters ({n_trainable}) exceed strict 60k limit.")
        return
    ...
    mp = f"{OUT_DIR}/v29_factorized_engine.pth"; cp = f"{OUT_DIR}/v29_config.json"
    torch.save(model.to("cpu").state_dict(), mp)
    cfg = {
        "version": "29.0", 
        "architecture": "Factorized Epidemic Potential + Calibration",
    ...
    torch.save(model.state_dict(), f"{DRIVE_DIR}/v29_factorized_engine.pth")
    logger.info(f"✅ V29 Engine saved to {mp} and Drive.")
